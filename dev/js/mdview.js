                  ...
                  if (_this.option.mode == 'include' && _this.option.spa == true) {
                    ...
                    for (i = 0, ln = link_array.length; i < ln; i++) {
                      ...
                      if (href.match(/^(?!http(|s)).*/) && href.match(/^(?!\#).*/) && href.match(/^(?!.*(\/|=)).*/) && href.split('.').pop() == 'md') {
                        link_array[i].href = "?" + _this.option.link_target + "=" + href; //if(this.id !== this.option.link_target){continue}
                ...
                if (_this.option.frontmatter == true) {
                  _this.renderer.use(front_matter_plugin, {
                      callback: (fm, token, state) => {
                        if(fm){
                          window.MDView[_this.id].meta = YAML.parse(fm);
                        }
                      }
                  })
                  /*
...
/*! yaml.js -- Copyright (c) 2010 Jeremy Faivre -- license (MIT): https://github.com/jeremyfa/yaml.js/blob/develop/LICENSEs */
