(function () {
  ...
  /*! markdown-it 10.0.0 https://github.com//markdown-it/markdown-it @license MIT */(function (f) {
  ...
  /*! markdown-it-footnote 3.0.2 https://github.com//markdown-it/markdown-it-footnote @license MIT */(function (f) {
    if ((typeof exports === "undefined" ? "undefined" : _typeof(exports)) === "object" && typeof module !== "undefined") {
      module.exports = f();
    } else if (typeof define === "function" && define.amd) {
      define([], f);
  ...
  function markdownItYamlPlugin(md, options) {
    md.block.ruler.before('code', 'yaml', createYamlRuleBlock(options));
  }
  /** RuleBlock Parser for MarkdownIt */
  function createYamlRuleBlock() {
    var options = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : {};
    return function yamlRuleBlock(state, start, end, silent) {
      var _a;
      ...
      var content = state.src.slice(state.bMarks[start + 1], state.bMarks[yamlEnd]);
      // No output currently
      // if (!silent) {
      ...
      //   token.block = true;
      // }
      var meta = load(content, {
        json: true
      }) || {};
      (_a = options.cb) === null || _a === void 0 ? void 0 : _a.call(options, meta, content);
      state.line = yamlEnd + 1;
      return true;
    };
  }
  function getStr(state, line) {
  ...
  */
  ...
  var MarkdownViewer = /*#__PURE__*/function (_HTMLElement) {
    ...
    function MarkdownViewer() {
      ...
      _defineProperty(_assertThisInitialized(_this), "SetDefaults", function () {
        _this.option = {};
        if (_this.dataset.option) {
          _this.option = JSON.parse(_this.dataset.option);
        } else {
          if (_this.dataset.html) {
            _this.option.html = JSON.parse(_this.dataset.html);
          }
          if (_this.dataset.sanitize) {
            _this.option.sanitize = JSON.parse(_this.dataset.sanitize);
          }
          if (_this.dataset.format) {
            _this.option.format = _this.dataset.format;
          }
          if (_this.dataset.spa) {
            _this.option.spa = JSON.parse(_this.dataset.spa);
          }
          if (_this.dataset.link_target) {
            _this.option.link_target = _this.dataset.link_target;
          }
          if (_this.dataset.frontmatter) {
            if (_this.dataset.frontmatter === "true") {
              _this.option.frontmatter = true;
            } else {
              _this.option.frontmatter = false;
            }
          }
        }
        if (_this.option.html == undefined) {
          _this.option.html = false;
        }
        if (_this.option.sanitize == undefined) {
          _this.option.sanitize = true;
        }
        if (_this.option.format == undefined) {
          _this.option.format = "markdown";
        }
        if (_this.option.spa == undefined) {
          _this.option.spa = false;
        }
        if (_this.option.link_target == undefined) {
          _this.option.link_target = _this.id;
        }
        if (_this.option.frontmatter == undefined) {
          _this.option.frontmatter = true;
        }
      ...
      _defineProperty(_assertThisInitialized(_this), "UnescapeEntity", function (str) {
        return str.replace(/&amp;/g, '&').replace(/&gt;/g, '>').replace(/&lt;/g, '<').replace(/&quot;/g, '\"').replace(/&#039;/g, '\'');
      });
      _defineProperty(_assertThisInitialized(_this), "QueryDecoder", function () {
        var query = [];
              ...
              switch (_context.prev = _context.next) {
                case 0:
                  ...
                  if (path_array.length > 1) {
                    path_array.pop();
                    _this.option.default_path = path_array.join("/") + "/";
                  }
                  if (!target) {
                    _context.next = 14;
                    break;
                ...
                case 36:
                  ...
                  if (_this.option.mode == 'include' && _this.option.spa == true) {
                    ...
                    for (i = 0, ln = link_array.length; i < ln; i++) {
                      ...
                      if (href.match(/^(?!http(|s)).*/) && href.match(/^(?!\#).*/) && href.match(/^(?!.*(\/|=)).*/) && href.split('.').pop() == 'md') {
                        link_array[i].href = "?" + _this.option.link_target + "=" + href;
                        //if(this.id !== this.option.link_target){continue}
                        link_array[i].addEventListener('click', function (e) {
                          e.preventDefault();
                          var file = e.currentTarget.href.split("=")[1];
                          var url = "?" + _this.option.link_target + "=" + file;
                          var target_element = document.querySelector("#".concat(_this.option.link_target));
                          _this.status = "reloading";
                          target_element.dataset.status = "reloading";
                          _this.load(file);
                          window.history.pushState(null, null, url);
                          _this.Storage.History.push({
                            url: url,
                            file: file
                          });
                          _this.setAttribute("src", _this.option.default_path + file);
                        });
                      }
                    }
                  }
                  ...
                  loading_target_section = loading_target.querySelector("section");
      ...
      _defineProperty(_assertThisInitialized(_this), "init", /*#__PURE__*/_asyncToGenerator( /*#__PURE__*/_regeneratorRuntime().mark(function _callee2() {
        return _regeneratorRuntime().wrap(function _callee2$(_context2) {
          while (1) {
            switch (_context2.prev = _context2.next) {
              case 0:
                _this.renderer = window.markdownit({
                  html: _this.option.html,
                  linkify: true,
                  breaks: true,
                  typographer: true,
                ...
                if (_this.option.frontmatter == true) {
                  _this.renderer.use(markdownItYamlPlugin, {
                    cb: function cb(json, row) {
                      if (json) {
                        window.MDView[_this.id].meta = json;
                      }
                    }
                  });
                }
                ...
                if (_this.option.mode == 'include' && _this.option.spa == true) {
                  GrobalStorage.popstate = window.addEventListener("popstate", function () {
                    ...
                    if (q[_this.id]) {
                      _this.dataset.status = "reloading";
  ...
  var MDViewPlugin = /*#__PURE__*/function (_HTMLElement2) {
    _inherits(MDViewPlugin, _HTMLElement2);
    var _super2 = _createSuper(MDViewPlugin);
    function MDViewPlugin() {
      var _this2;
      _classCallCheck(this, MDViewPlugin);
      _this2 = _super2.call(this);
      var plugins = _this2.children;
      _this2.dataset.count = plugins.length;
      _this2.dataset.loaded = 0;
      _this2.dataset.status = "assigned";
      return _this2;
    }
  ...
  var MDViewPluginToc = /*#__PURE__*/function (_HTMLElement3) {
    _inherits(MDViewPluginToc, _HTMLElement3);
    var _super3 = _createSuper(MDViewPluginToc);
    function MDViewPluginToc() {
      var _this3;
      _classCallCheck(this, MDViewPluginToc);
      _this3 = _super3.call(this);
      _defineProperty(_assertThisInitialized(_this3), "crc32", function (str) {
      ...
      _defineProperty(_assertThisInitialized(_this3), "init", function () {
        _this3.renderer = window.markdownit({
          html: true,
          linkify: true,
          breaks: true,
          typographer: true
        });
        var mdview_content = _this3.closest('mdview-content');
        ...
        for (var i in header_array) {
          if (header_array[i].textContent) {
            header_array[i].id = _this3.crc32(header_array[i].textContent);
            var indent_seeds = "          ";
        ...
        var toc_html = _this3.renderer.render(toc);
        var toc_array = mdview_content.querySelectorAll(".toc");
        var toc_title;
      ...
      _this3.dataset.status = "assigned";
      return _this3;
    }
    _createClass(MDViewPluginToc, [{
      key: "connectedCallback",
      value: function connectedCallback() {
        this.dataset.status = "connected";
  ...
  var MDViewPluginHighlight = /*#__PURE__*/function (_HTMLElement4) {
    ...
    _createClass(MDViewPluginHighlight, [{
      ...
      value: function connectedCallback() {
        ...
        var scripts = ["https://cdn.jsdelivr.net/gh/highlightjs/cdn-release@latest/build/highlight.min.js", "https://cdn.jsdelivr.net/npm/highlightjs-line-numbers.js@latest/dist/highlightjs-line-numbers.min.js"];
        var styles = ["https://cdn.jsdelivr.net/gh/highlightjs/cdn-release@latest/build/styles/".concat(highlight_style, ".min.css")];
